...
if (isset($_POST['btnActualizar'])) {
    ...
    if($update_estado->execute()){
        if($estado_factura == 'Pagado'){
            ...
            header("Location: ../empleado/facturas.php?success=true&message=El+estado+de+la+factura+se+ha+actualizado+correctamente");
            exit;
        }
    }else{
        $alerta = "<script>
        Swal.fire({
            ...
            icon: 'error',
            didClose: () => { 
                window.location.href = '../pages/empleado/facturas.php';
            }
        });   
        </script>";
    }
