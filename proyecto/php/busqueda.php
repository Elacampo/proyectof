<?php
// Verificar si se recibió la categoría seleccionada
if(isset($_GET['categoria_seleccionada'])) {
    ...
    $sql = $conectar->prepare("SELECT p.id, p.nombre_producto, p.imagen_producto, p.precio_producto, p.categoria 
    FROM producto p 
    INNER JOIN stock s on p.id = s.producto
    WHERE p.categoria = ?");
    
    $sql->bind_param("i", $categoria_seleccionada);
    $sql->execute();
    $resultado = $sql->get_result(); // Obtener el conjunto de resultados
    ...
    if($resultado->num_rows > 0) {
        ...
        while($fila = $resultado->fetch_assoc()) {
            ...
            echo '<div class="px-4 py-3 w-72">';
            echo '<span class="text-gray-400 mr-3 uppercase text-xs">'. $categoria_resultado['nombre'] .'</span>';
            ...
            echo '<a href="ver_producto.php?id='.$fila['id'].'" class="mr-4 icon-link" title="Ver más">'; // Agregar la clase 'icon-link' para el icono del ojo
            ...
            echo '<a href="#" title="Añadir al carrito" class="icon-link">'; // Agregar la clase 'icon-link' para el icono del carrito
            echo '<svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">';
        ...
        <div>
          <span class="font-medium">Error!</span> No se encontraron resultados para la categoría seleccionada
        </div>
      </div>';
        echo '</section>';
    ...
    // No se recibió la categoría seleccionada
    echo '<p>No se proporcionó una categoría para buscar productos.</p>';
}
?>
